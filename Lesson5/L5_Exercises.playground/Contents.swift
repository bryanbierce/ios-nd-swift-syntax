...
func getLastLetterOf(_ word: String) -> Character {
    var word = word
    return word.remove(at: word.index(before: word.endIndex))
}
//:Test out your discovery below by returning the last letter of the String, "bologna".
var word = "bologna"
getLastLetterOf(word)
//: __Problem 2__
//:
...
func combineLastCharacters(collection: String, word: String) -> String {
    var collection = collection
    collection.append(getLastLetterOf(word))
    return collection
}
let nonesenseWord = nonsenseArray.reduce("", combineLastCharacters)
print(nonesenseWord)
...
func getDigitsIn(_ word: String) -> Bool {
...
func cleanWords(_ array: [String]) -> [String] {
    func notFourLetters(_ word: String) -> Bool {
        return word.characters.count != 4
    }
    return array.filter(notFourLetters)
}
print(cleanWords(dirtyWordsArray))
//: __Problem 5__
//:
...
class MovieArchive {
    func filterByDirector(movies: [String: String], director: String) -> [String] {
        func hasDirector(collection: [String], movie: (name: String, director: String)) -> [String] {
            return movie.director == director ? collection + [movie.name] : collection
        }
        return movies.reduce([], hasDirector)
    }
}
