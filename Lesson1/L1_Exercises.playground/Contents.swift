...
let randomNoun1 = Int(arc4random() % 9)
let randomNoun2 = Int(arc4random() % 9)

let yourSentence = "The \(nounArray[4]) stared at the \(nounArray[8])"
let yourSillySentence = "The \(nounArray[randomNoun1]) danced with the \(nounArray[randomNoun2])"
//: ### Exercise 2
...
let word = "fortunate"
let contains = word.contains("tuna") ? "contains" : "does not contain"
...
for (word, icon) in dictionary {
     newestString = newestString.replacingOccurrences(of: word, with: icon)
}
print(newestString)
//: ### Exercise 8
...
//desired output = "$5.67"
print("$\(Double(numOfPennies) / 100.00)")
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
...
let maximumTimeAllowed: Float64
